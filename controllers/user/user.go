...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	// 查询账号密码是否正确
	exist := models.IsExist(db, collection, bson.M{"username": user.UserName, "password": user.Password})
	if exist {
		// 登入往user表插入token
		token, _ := auth.GenerateToken(&user)
		err := models.FindOne(db, collection, bson.M{"username": user.UserName}, nil, &user)
		// fmt.Println(user)
		user.Token = token
		err = models.Update(db, collection, bson.M{"username": user.UserName}, user)
		if err != nil {
